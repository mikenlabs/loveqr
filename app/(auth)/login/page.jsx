...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('loveqr_token', data.token)
      localStorage.setItem('loveqr_user', JSON.stringify(data.user))
    ...
    <main className="min-h-screen bg-gradient-heart flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-sm animate-fade-in-up">
        <div className="text-center mb-8">
          ...
          <p className="text-love-400 font-heading text-lg">Welcome back</p>
        </div>
        <form onSubmit={handleSubmit} className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl border border-love-200/50 p-8 space-y-5">
          ...
          <p className="text-center text-sm text-love-400">
            ...
            <Link href="/register" className="text-love-600 hover:text-love-800 font-semibold">Sign up</Link>
          </p>
